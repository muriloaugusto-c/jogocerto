...
export class ReservarComponent {
  ...
  getTotalQuadrasAlugadas(){
    ...
    this.http
      .get<any>('https://rightgame-api.onrender.com/users/reports/courtUsageStats', {headers})
        .subscribe({
          next: (data: any) => {
  ...
  getTotalQuadrasAlugadasPorMes(){
    ...
    const headers = {'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` };
    this.http
      .get<any>('https://rightgame-api.onrender.com/users/reports/courtUsageStatsByMonth', {headers})
        .subscribe({
          next: (data: any) => {
            this.dataTotalQuadrasAlugadasPorMes = data.courtUsageStats
          },
          error: (error: any) => {
            console.log('There was an error!', error);
          },
